class Employee {
  ...
  const wiproEmpolyees=arrayEmployees.filter((employee)=>{
    return employee.emp_company===`Wipro`;
  });
  let salarysum=0;
  for (const employee of wiproEmpolyees) {
    salarysum=salarysum+employee.emp_salary;
  }
  console.log(`Average salary from Wipro:${salarysum/wiproEmpolyees.length}`);
  console.log(`-----------------------------------------------------------`);
  const wiproorinfyEmpolyees=arrayEmployees.filter((employee)=>{
    return employee.emp_company===`Wipro`||employee.emp_company===`Infy`;
  });
  let salarysum1=0;
  for (const employee of wiproorinfyEmpolyees) {
    salarysum1=salarysum1+employee.emp_salary;
  }
  console.log(`Average salary from Wipro or Infy:${salarysum1/wiproorinfyEmpolyees.length}`);